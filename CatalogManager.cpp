...
int CatalogManager::insertRecord(string tableName, int addNum)
{
    fileNode *ftmp = bm.getFile(tableName.c_str());
    blockNode *btmp = bm.getBlockHead(ftmp);
    ...
    if (btmp)
    {
        ...
        *originalRecordNum += (int)addressBegin;
        bm.set_dirty(*btmp);
...
int CatalogManager::setIndexOnAttribute(string tableName,string AttributeName,string indexName)//addressBegin指向一条记录开始
{
    fileNode *ftmp = bm.getFile(tableName.c_str());
    blockNode *btmp = bm.getBlockHead(ftmp);
